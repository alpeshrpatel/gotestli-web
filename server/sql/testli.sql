...
) ENGINE=InnoDB AUTO_INCREMENT=48357 DEFAULT CHARSET=utf8mb3;
...
-- testli.org_user_rerelation definition

CREATE TABLE `org_user_rerelation` (
  ...
  KEY `user_id` (`user_id`),
  KEY `org_id` (`org_id`),
  CONSTRAINT `org_user_rerelation_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`),
  CONSTRAINT `org_user_rerelation_ibfk_2` FOREIGN KEY (`org_id`) REFERENCES `organisation` (`id`)
...
CREATE TABLE `question_master` (
  ...
  `complexity` enum('easy','normal','hard','medium','killer','danger') DEFAULT NULL,
  ...
  CONSTRAINT `question_master_ibfk_1` FOREIGN KEY (`question_type_id`) REFERENCES `question_type` (`id`)
) ENGINE=InnoDB AUTO_INCREMENT=147 DEFAULT CHARSET=utf8mb3;

-- testli.question_options definition

CREATE TABLE `question_options` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `question_id` int DEFAULT NULL,
  `question_option` varchar(1000) DEFAULT NULL,
...
-- testli.question_set_categories definition
...
-- testli.question_set_questions definition
...
-- testli.states definition

CREATE TABLE `states` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `states_name` varchar(50) DEFAULT NULL,
...
-- testli.user_test_result_dtl definition

CREATE TABLE `user_test_result_dtl` (
  `id` int unsigned NOT NULL AUTO_INCREMENT,
  `user_test_result_id` int DEFAULT NULL,
  `question_set_question_id` int DEFAULT NULL,
  ...
  `modified_by` int DEFAULT NULL,
  `modified_date` datetime DEFAULT CURRENT_TIMESTAMP,
  `status` int unsigned NOT NULL COMMENT 'Question status  - 0- Not Submitted,  1- Submitted,  2 - Review , 3 - Review & Submitted',
  PRIMARY KEY (`id`),
  KEY `user_test_result_dtl_user_test_result_FK` (`user_test_result_id`)
) ENGINE=InnoDB AUTO_INCREMENT=1712 DEFAULT CHARSET=latin1;
