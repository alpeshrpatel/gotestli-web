...
const pageMetadata = {
  title: "Instructors | Best Quiz Maker App Admin Panel – GoTestli",
  description:
    "List of Instructors on GoTestli, the best quiz app for learning and trivia. Process payments, track refund statuses, approve or deny requests, and maintain complete financial records for subscriptions and quiz purchases.",
  keywords:
    "best quiz app for learning, best quiz maker app, quiz app for trivia, gotestli instructors, gotestli instructors instructor management admin, instructors list, list of instructors, instructor management system, quiz platform instructors, student tracking, student administration, quiz app users",
  canonical: "https://gotestli.com/admin/instructors",
  category: "Financial Administration, instructors Management, Quiz Platform",
  subject: "instructors Management, Quiz App Users, Student Tracking, Financial Administration",
  audience: "Platform Administrators, Customer Service Teams, Accountants,instructors, Quiz App Users, Educators, Trainers, instructors"
};
    ...
    async function getPayments(page = 1, rowsPerPage = 10) {
        ...
        try {
            if (token) {
                let url = ''
                if (searchQuery) {
                    url = `/api/users/admin/instructors/list/?start=${start}&end=${end}&search=${encodeURIComponent(searchQuery)}&orgid=${orgid}`;
                } else {
                    url = `/api/users/admin/instructors/list/?start=${start}&end=${end}&orgid=${orgid}`
                }
    ...
    const handleApprove = async (payment, status) => {
        try {
            if (token) {
                ...
                const response = await API.put(`/api/instructors/refund/${payment.payment_intent_id}`, {
                    is_delete: 1
                }, {
                    headers: {
                        Authorization: `Bearer ${token}`,
                 ...
                 <Sidebar />
                        ...
                        {instructors.length > 0 ? (
                            <div className="table-responsive mt-1" style={{ paddingTop: "20px" }}>
                                ...
                                //  tableData={filteredData.length > 0 ? filteredData : instructors}
                                />
                            </div>
                        ) : (
                            <h4 className="no-content text-center">
                                Not any Instructor found! 🌟
                            </h4>
                        )}
