...
const StudyMaterialMegamenu = ({ allClasses }) => {
    ...
    const isMobile = allClasses && allClasses.includes('mobile');
    

    return (
        <div className={`${allClasses ? allClasses : ""} position-absolute`} >
            <Link
                ref={linkRef}
                to="#"
                onClick={() => setExploreActive((prev) => !prev)}
                className={`d-flex align-items-center ${isMobile ? 'text-black': 'text-white'} text-nowrap`}
                style={{ 
            ...
            >
                <i className="icon icon-online-learning mr-2"></i>
                Study Material
                {isMobile && <i className="icon-chevron-down text-13 ml-10"></i>}
            </Link>
            ...
            {exploreActive && (
                ...
                >
                    {menuData.map((mainItem, i) => (
                        <div key={i} style={{ marginBottom: isMobile ? "20px" : "0" }}>
                            <h6 className="fw-bold text-primary mb-2" style={{ fontSize: isMobile ? "16px" : "14px" }}>
                                {mainItem.title}
                            </h6>
                            {mainItem.subItems.map((subItem, j) => (
                                <div key={j} className="mb-3">
                                    <p className="mb-1 text-dark fw-semibold" style={{ fontSize: isMobile ? "14px" : "13px" }}>
                                        {subItem.title}
                                    </p>
                                    <ul className="list-unstyled ps-3">
                                        {subItem.subItems.map((item, k) => {
